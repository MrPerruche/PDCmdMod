...
--
--   give name <IA_asset_name> [amount]
--     e.g. give name IA_Resource_Raw_Lead 5
--
--   give display [index] <display_name> [amount]
--     e.g. give display lead platelet 5
--     e.g. give display 2 lead platelet 5   <- index if multiple matches
--
-- Notes:
--   - display mode matches case-insensitively, ignoring spaces/punctuation/dashes
--   - display mode index loops (so index 3 of 2 results = result 1)
...
-- Mode: name
-- ============================================================
local function HandleName(input, count)
    local all = FindAllOf("ItemArchetype")
    if not all then
    ...
    if #paths == 0 then
        uim.sendMessage("Give", "No loaded archetype matching name: " .. input, uim.MessageTypes.LOGS)
        SendMessageBadName(input)
        --print("[Give] ERROR: No loaded ItemArchetype found with name: " .. input)
...
-- ============================================================
local function HandleDisplay(params)
    ...
    if params[startIdx] and IsInt(params[startIdx]) then
        index = math.floor(tonumber(params[startIdx]))
        startIdx = startIdx + 1
    end
 
    if endIdx >= startIdx and params[endIdx] and IsInt(params[endIdx]) then
        count = math.floor(tonumber(params[endIdx]))
        endIdx = endIdx - 1
    end
 
    if startIdx > endIdx then
        uim.sendMessage("Give", "Invalid syntax.", uim.MessageTypes.ALERT)
        uim.sendMessage("Give", "Usage: give display [index] <display_name> [amount]", uim.MessageTypes.CHATLIKE)
        --print("[Give] ERROR: No display name provided")
        --print("[Give] Usage: give display [index] <display_name> [amount]")
        return
    end
 
    local nameParts = {}
    for i = startIdx, endIdx do
        table.insert(nameParts, params[i])
    end
    local rawName = table.concat(nameParts, " ")
    local normalizedInput = Normalize(rawName)
    uim.sendMessage("Give", "Searching for: '" .. rawName .. "'", uim.MessageTypes.LOGS)
    -- print("[Give] Searching for: '" .. rawName .. "' (normalized: '" .. normalizedInput .. "')")
    ...
    if #matches == 0 then
        ...
        --print("[Give] Tip: try 'give name' or 'give path' if the asset isn't loaded yet")
        return
    end
...
local function HandleTips()
    uim.sendMessage("Give", "Use 'give help' for usage instructions.\n"
                        ...
                        ..  "The 'give name' mode expects inputs such as IA_Resource_Raw_Lead, not display names like 'Lead Platelet'.\n"
...
RegisterConsoleCommandHandler("give", function(FullCommand, Parameters)
    ...
    elseif mode == "name" then
        ...
        local count = tonumber(Parameters[3]) or 1
        if count < 1 then count = 1 end
        HandleName(input, count)
 
    elseif mode == "display" then
        local displayParams = {}
        for i = 2, #Parameters do
            table.insert(displayParams, Parameters[i])
        end
        HandleDisplay(displayParams)
    ...
    elseif mode == "help" then
        ...
        uim.sendMessage("Give", "Valid modes: fullpath, path, name, display, help, tips", uim.MessageTypes.CHATLIKE)
    end
